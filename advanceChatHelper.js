...
function receiveprivateMessage(senderId, senderName, receiveId, message, chatId) {
    alert("receiveprivateMessage before" + message);
    connection.invoke("DecryptPrivateMessage", senderId, message, "").catch(function (err) {
        alert('Send Private Message', message)
       
        return console.error(err.toString());
    });
    alert("receiveprivateMessage after"+message);
